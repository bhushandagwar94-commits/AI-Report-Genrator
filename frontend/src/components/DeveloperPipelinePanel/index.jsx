...
export default function DeveloperPipelinePanel({
  pipelineDebug,
  isOpen,
  setIsOpen,
}) {
  ...
  const isDev =
    ...
    import.meta.env.VITE_SHOW_PIPELINE_DEBUG === "true";
  if (!isDev) return null;

  const debug =
    pipelineDebug && typeof pipelineDebug === "object" ? pipelineDebug : {};
  const functionBlocks = Array.isArray(debug.functionBlocks)
    ? debug.functionBlocks
    : [];
  const providerAttempts = Array.isArray(debug.providerAttempts)
    ? debug.providerAttempts
    : Array.isArray(debug.aiModels?.providerAttempts)
      ? debug.aiModels.providerAttempts
      : [];
  const aiNodes = Array.isArray(debug.aiNodes)
    ? debug.aiNodes
    : Array.isArray(debug.aiModels?.aiNodes)
      ? debug.aiModels.aiNodes
      : [];
  const prompts = Array.isArray(debug.prompts) ? debug.prompts : [];
  const calculationTrace = Array.isArray(debug.calculationTrace)
    ? debug.calculationTrace
    : [];
  const inputSummary =
    debug.inputSummary && typeof debug.inputSummary === "object"
      ? debug.inputSummary
      : {};
  const validationTrace =
    debug.validationTrace && typeof debug.validationTrace === "object"
      ? debug.validationTrace
      : {};
      ...
      <div className="flex items-center justify-between p-4 border-b border-white/10 bg-slate-800">
        ...
        <div className="flex items-center gap-x-3">
          ...
          <button
            ...
            className="text-white/60 hover:text-red-400 p-1 rounded-md"
          >
            <X size={20} />
          </button>
        </div>
        ...
        {(() => {
            ...
            return (
              <div className="flex flex-col gap-2">
                <div className="text-[12px]">
                  <strong>Status:</strong> {debug.status || "unknown"}
                </div>
                {debug.fallbackReason && (
                  <div className="text-[12px] text-red-400">
                    <strong>Fallback Reason:</strong> {debug.fallbackReason}
                  </div>
                )}
                {errors.length > 0 && (
                  <div className="text-[12px] text-red-400">
          ...
          if (activeTab === "aiModels") {
            ...
            if (!aiStatus) {
              return (
                <div className="text-white/30 italic">
                  AI enhancement status was not returned by backend.
                </div>
              );
            }
            ...
            return (
              <div className="flex flex-col gap-4">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="mt-1 space-y-1">
                      ...
                      <div>
                        <span className="opacity-70">Final Enhancer Used:</span>{" "}
                        {aiStatus.finalEnhancerUsed}
                      </div>
                      {aiStatus.failureReason && (
                        <div>
                      ...
                      {aiStatus.developerMessage && (
                        <div>
                          <span className="opacity-70">Developer Message:</span>{" "}
                ...
                {aiStatus.droppedFields &&
                  aiStatus.droppedFields.length > 0 && (
                    <div>
                      <h3 className="font-bold mb-1 opacity-80 text-yellow-400">
                        Dropped Fields
                      </h3>
                ...
                {aiStatus.providerAttempts &&
                aiStatus.providerAttempts.length > 0 ? (
                  <div>
                    <h3 className="font-bold mb-1 opacity-80">
                      Provider Attempts
                    </h3>
                    <div className="bg-black/30 p-2 rounded max-h-64 overflow-y-auto text-[11px] font-mono leading-relaxed">
                      {aiStatus.providerAttempts.map((attempt, i) => (
                        ...
                        >
                          ...
                          {attempt.model}
                          {attempt.error && (
                            <div className="text-red-400 mt-1 opacity-80 text-[10px]">
                              Error: {attempt.error}
                            </div>
                          )}
                          {attempt.reason && !attempt.error && (
                            <div className="text-yellow-400 mt-1 opacity-80 text-[10px]">
          ...
          if (activeTab === "prompts") {
            if (prompts.length === 0) {
              return (
                <div className="text-white/30 italic">
                  No prompts recorded for this run.
                </div>
              );
            }
            return (
          ...
          if (
            debug[activeTab] === undefined ||
            (Array.isArray(debug[activeTab]) && debug[activeTab].length === 0)
          ) {
            return (
              <div className="text-white/30 italic">
