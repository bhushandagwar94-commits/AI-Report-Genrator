    ...
    if (pDebug.providerAttempts) {
      ...
      if (g.length > 0)
        geminiAttempts = g
          .map(
            (a) =>
              a.model + " (" + a.status + ") - " + (a.durationMs || 0) + "ms"
          )
          .join(", ");

      const o = pDebug.providerAttempts.filter(
        (a) => a.provider === "openrouter"
      );
      if (o.length > 0)
        openRouterAttempts = o
          .map(
            (a) =>
              a.model + " (" + a.status + ") - " + (a.durationMs || 0) + "ms"
          )
...
`;
    ...
    console.log(output);
    fs.writeFileSync(
      path.join(__dirname, "pipeline_debug_out.json"),
      JSON.stringify(pDebug, null, 2)
    );
  } catch (err) {
    if (err.response) {
