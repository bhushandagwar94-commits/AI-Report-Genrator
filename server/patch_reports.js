...
code = code.replace(/function safeBuildPipelineDebug[\s\S]*?errors: \["Pipeline debug generation crashed"\]\n        \}\);\n/, '');
...
code = code.replace(/\/\/ Build pipelineDebug object[\s\S]*?pipelineDebug generation crashed"\]\n\s*\}\);/g, '');
...
code = code.replace(/providerAttempts = e\.providerAttempts \|\| providerAttempts \|\| \[\];/g, '');

code = code.replace(/let providerAttempts = \[\];/g, '');
code = code.replace(/providerAttempts,/g, 'providerAttempts: debugCollector.data.providerAttempts,');
...
code = code.replace(/aiFinalizationTimeoutMs,/g, 'debugCollector.data.config.aiFinalizationTimeoutMs,');
